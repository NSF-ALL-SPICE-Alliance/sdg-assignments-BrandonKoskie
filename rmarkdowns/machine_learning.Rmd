...
```{r}
rf_energy <- randomForest(normalized_score_sdg7_elecac ~ ., data = sdr_data_imputed, importance = TRUE)
```
...
```{r}
importance_df <- as.data.frame(rf_energy$importance)
```
...
ggplot(importance_df_top_10, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) +
  geom_bar(stat = "identity", fill = "steelblue", color = "black") +
  theme_minimal() +
  labs(title = "Most Important Variables in Predicting Affordable/Clean Energy",
       subtitle = "Top 10",
       y = "SDG Indicator",
       x = "Feature Importance (% Increase in Mean Squared Error)")
```
...
```{r}
pd <- pdp::partial(rf_energy, pred.var = c("normalized_score_sdg6_sanita", "normalized_score_sdg6_water"))
# Default PDP
plotPartial(pd)
```
