...
```
```{r}
goal_scatterplot<-ggplot(data = sdr_data, aes(x = goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  geom_point() +
  theme_minimal() +
  scale_fill_viridis_d() +
...
mytext <- paste(
    "Country: ", sdr_data_world_joined$country,"<br/>", 
    "Goal 14 Score: ", round(sdr_data_world_joined$goal_14_score, 2), 
    sep="") %>%
  lapply(htmltools::HTML)
...
leaflet(sdr_data_world_joined) %>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=2) %>%
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("magma", goal_14_score)(goal_14_score), label = mytext)
```
