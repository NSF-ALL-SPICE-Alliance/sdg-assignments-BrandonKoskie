...
```{r}
ggplot(data = sdr_data, aes(x= goal_6_score, y = goal_7_score, color = regions_used_for_the_sdr)) +
  geom_point()
```
...
Interpretation: The scatter plot shows that most countries scoring high in goal 8 are also scoring well in goals 6 and 7. Another observation is that the lowest scoring countries in goals 6 and 7 also score low in 8. This shows a positive correlation with the goals because if a country scores well in one goal they are likely to reach a good score in the others.
...
```{r}
ggplot(sdr_data, aes(x = goal_14_score)) +
  geom_histogram()
```
```{r}
...
Interpretation: The highest combined score for the most countries appear to be between 60 and 67 for the goal with about 18 countries falling into the highest score. For the highest score overall it is about 95 with about 2 countries contributing to this.
